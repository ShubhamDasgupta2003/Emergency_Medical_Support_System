<?php
    ...
    if(isset($_POST['book_ride']))
    {
        ...
        if($update_result)
        {
            ...
            $insert_result = $db->insert('medtech_order',array("$invoice_no","$eid","$user_id","$name","$contno","$book_lat","$book_lon","$address","$lmark","$cur_date","$cur_time"));
            if($insert_result)
            {
                header("Location:invoice_mail.php?eid=$eid&ename=$ename$name=name&address=$address&billno=$invoice_no&contno=$contno");
            }
        }
    }
